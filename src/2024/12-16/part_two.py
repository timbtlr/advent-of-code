...
def walk_path(grid, start_row, start_col):
    stack = []
    heappush(stack, (0, start_row, start_col, ">", set()))
    visited = set()
    final_scores = defaultdict(lambda: set())
    
    while stack:
        score, row, col, direction, path = heappop(stack)
        path.add((direction, row, col))
        visited.add((direction, row, col))

        if grid[row][col] == "E":
            final_scores[score].update(path)
            continue
            ...
            for row_offset, col_offset, new_direction in DIRECTIONS:
                ...
                if grid[new_row][new_col] != "#":
                    heappush(stack, (new_score, new_row, new_col, new_direction, copy(path)))
        
    min_score = min(final_scores.keys())
    return len(list(set([(x, y) for _,x,y in final_scores[min_score]])))
...
def solve(file_location):
    ...
    grid, starting_location, _ = parse_input(file_input)
    print_grid(grid, [])
