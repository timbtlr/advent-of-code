...
def solve(file_location):
    ...
    a_reg_value = 0
    while True:
        if a_reg_value % 10000 == 0:
            print(a_reg_value)

        new_registers = copy(registers)
        new_registers["A"] = a_reg_value
        ...
        while index < len(program)-1:
            ...
            new_registers, jump_to, output = OPERATIONS[op](new_registers, operand)
...
print("Example", solve("example_simple.txt"))
