...
CARD_VALUE = {"A": 14, "K": 13, "Q": 12, "T": 10, "J": 1}


def get_hand_strength(cards):
    jack_count = len([i for i in cards if i == "J"])
    counts = collections.Counter([i for i in cards if i != "J"]).values()
    count_len = len(counts)

    if count_len == 1 or jack_count == 5:
        return 7

    if any([i + jack_count == 5 for i in counts]):
        return 7

    if count_len == 2:
        if any([i == 4 for i in counts]):
            return 6

        if any([i + jack_count == 4 for i in counts]):
            return 6

        return 5

    if count_len == 3:
        if any([i == 3 for i in counts]):
            return 4

        if any([i + jack_count == 3 for i in counts]):
            return 4
...
def solve(file_location):
    ...
    hand_order = []
    for hands in dict(sorted(bid_ranks.items())).values():
